...
#doc-roast 'operators','&infix:«eq»'
#doc-roast 'operators','&infix:«ne»'
{
  ok("a" eq "a",     "eq true");
  ok(!("a" eq "ab"), "eq false");
  ok("a" ne "ab",    "ne true");
...
#doc-roast 'operators','string comparison on undefined values'
{
  my $foo;
  ok($foo eq "",     "Any eq ''");
...
#doc-roast 'operators','Negated relational operators'
ok(2 !== 3,         "!== true");
ok(!(2 !== 2),      "!== false");
...
#doc-roast 'operators','numeric context on undefined values'
ok($foo == 0,      "Any == 0");
