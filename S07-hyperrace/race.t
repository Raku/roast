...
#?rakudo todo 'hyper and race cause lists to become empty RT #126597'
{
    ...
    is @result.sort, <A B C D E F G>, "can hypermap a simple code block";

    @result = (^50).list.hyper.map({ $_ * 2 });
    is @result.sort, (^50).map({$_*2}), "hyper over a big-ish list of Ints";
}
...
#?rakudo todo 'hyper and race cause lists to become empty RT #126597'
{
    ...
    is @result.sort, (50, 60, 70, 80, 90, 100), "hyper + grep";
}
#?rakudo todo 'hyper and race cause lists to become empty RT #126597'
{
    my @result = (^100).list.hyper.grep({ $_.is-prime }).map({ $_ * $_ });
    is @result.sort, (4, 9, 25, 49, 121, 169, 289, 361, 529, 841, 961, 1369, 1681, 1849, 2209, 2809, 3481, 3721, 4489, 5041, 5329, 6241, 6889, 7921, 9409), "hyper + grep + map";
}
