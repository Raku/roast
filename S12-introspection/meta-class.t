...
ok(Foo.HOW.isa(Foo), '... Foo is-a Foo (of course)');
ok(Foo.^isa(Foo, Foo), '... Foo is-a Foo (of course) (as class method)');
...
ok Foo.^isa()[0] === Foo, '... the isa() property returns Foo as the first parent class';
...
is @methods[0].name eq 'bar', '... our first method is bar';
ok @methods[0].signature ~~ /'$param'/, '... our first methods signature is $param';
