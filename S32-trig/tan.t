...
# Num tests
is_approx(tan((-6.28318530723787).Num), -5.82873520418806e-11, "tan(Num) - -6.28318530723787");
...
# Complex tests
is_approx(tan((0.785398163397448 + 2i).Complex), 0.0366189934737061 + 0.9993292997396i, "tan(Complex) - 0.785398163397448 + 2i");
is_approx(tan(:x((2.35619449019234 + 2i).Complex)), -0.0366189934736279 + 0.999329299737467i, "tan(:x(Complex)) - 2.35619449019234 + 2i");

# Str tests
is_approx((3.14159265361894).Str.tan, 2.91436760209403e-11, "Str.tan - 3.14159265361894");
...
for TrigTest::sines() -> $angle
{
    ...
    is_approx($desired-result.Num.atan.tan, $desired-result, 
              "Num.atan - {$angle.num(Radians)}");
    
    # Num.atan(Complex) tests -- thorough
    for ($desired-result + 0i, $desired-result + .5i, $desired-result + 2i) -> $z {
        is_approx(tan(atan($z)), $z, 
                  "atan(Complex) - {$angle.num(Radians)}");
        is_approx($z.atan.tan, $z, 
                  "Complex.atan - {$angle.num(Radians)}");
    }
}
